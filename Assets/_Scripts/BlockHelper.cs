...
public static class BlockHelper
{
        ...
        (ChunkData chunk, int x, int y, int z, MeshData meshData, BlockType blockType)
    {
        ...
        var maxZXChunkPos = (chunk.worldReference.mapSizeInChunks * chunk.chunkSize)-chunk.chunkSize;
        var maxVoxelPos = chunk.chunkSize - 1;
        ...
        foreach (Direction direction in directions)
        {
            ...
            if ((chunkPos.x==maxZXChunkPos)|| (chunkPos.z==maxZXChunkPos)||(chunkPos.x==0)||(chunkPos.z==0))
            {
                if ((z == 0&&chunkPos.z==0) || (x == 0&& chunkPos.x==0) ||(z == maxVoxelPos &&chunkPos.z==maxZXChunkPos) ||(x == maxVoxelPos && chunkPos.x==maxZXChunkPos) )
                {
                    meshData = GetFaceDataIn(direction, chunk, x, y, z, meshData, blockType);
     ...
     static MeshData GetFaceDataIn(Direction direction, ChunkData chunk, int x, int y, int z, MeshData meshData, BlockType blockType)
    {
     ...
     static Vector2[] FaceUVs(Direction direction, BlockType blockType)
    {
     ...
     static Vector2Int TexturePosition(Direction direction, BlockType blockType)
    {
